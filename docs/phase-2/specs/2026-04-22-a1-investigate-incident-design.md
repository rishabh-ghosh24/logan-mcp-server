...
- **`infra_error`** is an internal flag set by the branch when any non-Budget, non-field-variance exception is caught (cluster failure, timeline backend outage, unexpected ServiceError during entity discovery, transport timeout). The orchestrator maps `infra_error=True` to `"source_errors"` in `partial_reasons`. Branch continues with empty data for that sub-phase rather than aborting the whole branch, so other sub-phases still get tried.
- **Timeline failures** are always best-effort per design; any non-Budget exception during the timeline query maps to `"timeline_omitted"` (not `"source_errors"`), leaves `timeline = None`, and lets the branch complete normally.

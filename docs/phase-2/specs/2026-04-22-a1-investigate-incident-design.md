...
**Error-class discrimination (load-bearing).** The orchestrator and the branch collaborate so that each failure class maps to the intended `partial_reasons` value — not the closest-looking one. Without this discipline, the most common mid-investigation failure (budget exhaustion during drill-down) would silently get mislabeled as `"source_errors"`, and real infrastructure problems (5xxs, timeouts) would masquerade as `"entity_discovery_partial"`.

- **`BudgetExceededError`** is re-raised by `_drill_down_one_source` (not caught internally) so `asyncio.gather(return_exceptions=True)` captures it as a result object; the orchestrator matches `isinstance(result, BudgetExceededError)` *explicitly before* the generic `Exception` branch, and adds `"budget_exceeded"` — never `"source_errors"`.
- **Field-variance** (`ServiceError` with `code == "InvalidParameter"` and `"Invalid field"` in the message) is the *only* failure class that maps to `"entity_discovery_partial"`. Detected by a helper `_is_field_variance_error(exc)`. Other ServiceErrors (5xx, auth, rate-limit, etc.) during entity discovery are NOT downgraded to field-variance.
- **`infra_error`** is an internal flag set by the branch when a non-Budget, non-field-variance exception is caught in the **cluster** or **entity-discovery** sub-phases (cluster query failure, unexpected `ServiceError` during entity discovery, transport timeout during either). The orchestrator maps `infra_error=True` to `"source_errors"` in `partial_reasons`. Branch continues with empty data for that sub-phase rather than aborting the whole branch, so other sub-phases still get tried.
- **Timeline failures** have their own dedicated path — they do **not** set `infra_error`. Any non-Budget exception during the timeline query (including backend 5xxs and transport timeouts) maps to `"timeline_omitted"`, leaves `timeline = None`, and lets the branch complete normally. This is a deliberate design choice: the timeline is the lowest-value sub-phase of a drill-down, and a backend flake there should not imply "this source's whole investigation has infrastructure problems."

The goal is that `partial_reasons` is *actionable*: "budget ran out mid-run" requires a different remediation than "this tenancy is missing a standard field" or "the LA backend is having a bad day."

...
6. **Source-less queries (P0 limitation)** — queries without a `'Log Source' = ...` or `'Log Source' in (...)` filter cannot be probed (no source to count against). They return `confidence="low"`, `estimated_bytes=0`, and `estimated_cost_usd=None` (cost unavailable). The bytes and cost budget checks are therefore skipped for source-less queries, but the **query-count limit still applies**. This is an accepted P0 limitation; a full-scan cost estimate would require an OCI "explain" API that does not yet exist.

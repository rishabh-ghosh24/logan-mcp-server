...
| Phase 1 / Observation | Reads, Telegram notifications/reports, local report generation where safe | Saved searches, dashboards, tiles, alerts, learned behavior, sample uploads, email/ONS delivery |
...
| Create report/export artifact | Explicit consent unless it is an in-chat Telegram summary |
...
Telegram report generation can be used in Phase 0/1 through Hermes. Logan-side report/export tools are not day-one defaults; enable them only when the specific delivery/storage path has been reviewed:
